...
const DishModifier
		...
		= { DishModifier("triple deep-fried",BEFORE_MODLOC),
		    ...
		    DishModifier("cockroach dung speckled",BEFORE_MODLOC),
...
public :
  ...
  void		serve		(Chef&		chef,
		  ...
		  while  (getDishPtr() != NULL)
		  {
		    std::cout
			<< chef
		    	<< ": \"Would you clear the freakin' table!?!\""
			<< std::endl;
			...
			)
{
  ...
  //  PERHAPS A LOOP HERE
  for  (int i = 0;  i < NUM_DISHES_TO_DO;  i++)
    gourmandPtr->consume(table.eatFrom(*gourmandPtr));

  return(gourmandPtr /* CHANGE IF YOU WANT */);
}


//  PURPOSE:  To harass CSC 407 students into learning thread programming and
//	thread-safe producer-consumer buffer creation.  If 'argc >= 2' then
